...
train_disjoint_f = 'train_disjoint.txt'

validation_disjoint_f = 'validation_disjoint.txt'
...
def main():
    ...
    if Validation:
        print("Generating training and validation disjoint features tensor...")

        disjointSplit(train_triplets_file, train_disjoint_f, validation_disjoint_f)

        train_disjoint_tensors, train_disjoint_labels = buildTripletTensor(features, train_disjoint_f, gen_labels=True)
        validation_disjoint_tensors, validation_disjoint_labels = buildTripletTensor(features, validation_disjoint_f, gen_labels=True)


        print("Building model...")
        # Build model to process features
        x = x_in = Input(train_disjoint_tensors.shape[1:])
        x = Activation('relu')(x)
        x = Dropout(0.7)(x)
        x = Dense(1152)(x)
        x = Activation('relu')(x)
        ...
        checkpoint_path = "CheckpointsVal/cp.ckpt"
        ...
        if train:
            ...
            model.fit(x=train_disjoint_tensors, y=train_disjoint_labels, epochs=6, callbacks=[cp_callback])
        ...
        y_test = model.predict(validation_disjoint_tensors)
        y_test_thresh = np.where(y_test < 0.5, 0, 1)

        validation_disjoint_labels = np.matrix(validation_disjoint_labels).T
        y_diff = y_test_thresh - validation_disjoint_labels

        error_number = np.count_nonzero(y_diff)

        accuracy = (y_diff.size - error_number)/y_diff.size
        ...
        checkpoint_path = "CheckpointsPred/cp.ckpt"
        ...
        if train:
            ...
            model.fit(x = train_tensors, y = labels, epochs=6, callbacks=[cp_callback])
