...
def honolulu_reg(inner_model):
    ...
    inner_model.add(Dense(125))
    inner_model.add(Activation("tanh"))
    inner_model.add(Dropout(0.25))
    inner_model.add(keras.layers.BatchNormalization(
        axis=-1,
        momentum=0.99,
        epsilon=0.001,
...
def main():
    ...
    callback = tf.keras.callbacks.EarlyStopping(monitor='val_root_mean_squared_error', patience=50, restore_best_weights=True)

    print(' \nTraining homo-lumu model')
    ...
    output = model_honolulu.predict(X_test_ws)
